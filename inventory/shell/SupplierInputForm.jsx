...
function SupplierInputForm({ onSupplierInputValues }) {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    const selectedPlanTypes = Object.keys(planType).filter(
      (key) => planType[key]
    );
    const supplierInputValues = {
      ...supplierInput,
      offers_delivery: deliveryOffered,
      plan_type: selectedPlanTypes,
    };
  ...
  return (
    <div>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 mb-5">
          ...
          <div className="flex items-center gap-2">
            <label className="flex items-center gap-1">
              <Input
                ...
                name="picked"
                value="Yes"
                onClick={() => setDeliveryOffered(true)}
              />
              Yes
            </label>
            <label className="flex items-center gap-1">
              <Input
                ...
                defaultChecked
                value="No"
                onClick={() => setDeliveryOffered(false)}
              />
              No
            </label>
          </div>
        ...
        {deliveryOffered && (
          <div className="flex flex-col gap-1">
            <Label htmlFor="delivery_charges">
        ...
        <div className="flex flex-col gap-1">
          ...
          <select
            ...
            required
          >
            <option value="distance">Charged based on distance (km)</option>
            <option value="weight">Charged based on weight (kg)</option>
          </select>
        </div>
        <div className="flex flex-col gap-1">
      ...
      <Button className="w-fit" type="button" onClick={handleSubmit}>
        Save
      </Button>
    </div>
  );
